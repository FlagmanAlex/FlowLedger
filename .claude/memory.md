...
- **Семантические цвета (текст/фон/статусы) — только через переменные `tokens.css`**
  (`var(--positive)`, `var(--negative)`, `var(--text)`, `var(--text-secondary)`, `var(--surface)`,
  `var(--divider)` и т.п.), никогда не дублировать их сырым hex — иначе фикс на уровне токена (как
  светлая тема `--positive: #0d9488` вместо тёмного `#2fe6b8` — на светлом фоне контраст был
  1.34:1, практически не виден) не долетает до мест, которые продублировали старое значение
  напрямую. Это касается и inline-style/JS-пропов, не только CSS-файлов — recharts в
  `Reports.tsx` не воспринимает CSS-классы, но `var(--токен)` как строка работает и в SVG-
  атрибутах (`fill`), и в объектах `style`/`contentStyle`/`tick`; так и обнаружился этот баг —
  график тренда хардкодил `#2fe6b8`/`#1b2136`/rgba-тени тёмной темы напрямую, поэтому светлая
  тема на графике не переключалась вовсе. Единственное осознанное исключение —
  **категориальная палитра** (`client/src/lib/palette.ts`, `PALETTE`/`colorForId`) для
  кошельков/категорий/долгов/участников: она намеренно одинакова в обеих темах (кошелёк остаётся
  тем же оттенком и в тёмной, и в светлой теме — как цвета событий в календаре), используется
  только как фон цветного кружка/полосы-прогресса с фиксированным белым текстом поверх, а не как
  цвет текста на фоне страницы — WCAG-контраст там не зависит от темы, трогать не нужно.
